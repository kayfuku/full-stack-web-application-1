...
def delete_venue(venue_id):
  ...
  try:
    ...
    # This helps!
    print(sys.exc_info())
    print('Venue {0} (id: {1}) has been deleted.'.format(venue_to_delete.name, venue_to_delete.id))
    success = True
  except:
    db.session.rollback()
    ...
    # This helps!
    print(sys.exc_info())
    print('Venue {0} (id: {1}) could not be deleted.'.format(venue_to_delete.name, venue_to_delete.id))
  finally:
    db.session.close()
...
def show_artist(artist_id):
  ...
  # past_shows
  past_shows = Show.query.filter(
    Show.start_time <= datetime.datetime.now(), Show.artist_id == artist_id).all()
  data['past_shows'] = []
  for ps in past_shows:
...
def create_artist_submission():
  ...
  try:
    ...
    print('An error occurred. Artist ' + request.form['name'] + ' could not be listed.')
    db.session.rollback()
    traceback.print_exc()
